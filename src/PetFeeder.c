...
void initHw()
{
  ...
  //---------------------------------------------
     ...
     GPIO_PORTB_AFSEL_R |= AUGER_MASK;                    //AFSEL
...
void analogISR()
{
    uint32_t time = 0;
    float level = 0.0;
    time = WTIMER1_TAV_R;                       //Give 'time' the value of the Wide Timer (One Shot Timer)
...
int main(void)
{
    ...
    while(true)
    {
        ...
        if(isCommand(&data, "time", 2))
        {
            ...
            if(((HOURS < 24) && (MINS <= 59)) || ((HOURS == 0) && (MINS <= 59)))
            {
        ...
        else if(isCommand(&data, "feed", 5))
        {
            ...
            if((event < 10) && (hour < 24) && (mins < 60))            //If user enters event index and hours/mins in a valid range then execute
            {
                ...
                if(secondsCompare < HIB_RTCC_R)
                {
                    hour += 24;
                }

                writeEeprom((16 * event), event);       //index field
                writeEeprom((16 * event) + 1, duration); //duration
                writeEeprom((16 * event) + 2, PWM); //pwm
                writeEeprom((16 * event) + 3, hour); //hours
                writeEeprom((16 * event) + 4, mins); //minutes
                writeEeprom((16 * event) + 5, EventActive); //Event Activated == Active Flag is set to 1 saying the event is active (background)
        ...
        else if(isCommand(&data, "feed", 2))
        {
            ...
            if(deleteText != NULL && cmpStr(deleteText, "delete") == 0)
            {
                if(deleteEvent < 10)
                {
                    valid = true;
                    ...
                    {
                        char strr[40];
                        writeEeprom((16*deleteEvent)+i, 0xFFFFFFFF);
                        writeEeprom((16*deleteEvent)+5, 0x0); //Setting Active
        ...
        else if(isCommand(&data, "schedule", 0))
        {
            ...
            {
                if((readEeprom(16*eNum) == 0xFFFFFFFF) && (readEeprom(16*eNum+5) == 0))
                {
                    putsUart0("\nNo events scheduled.\n");
                }
                else if(EventActive == readEeprom(16*eNum+5))
                {
                    uint16_t NewHours = 0;
                    if(readEeprom(16*eNum+3) > 24)
                    {
                        NewHours = readEeprom(16*eNum+3) % 24;
                    }
                    else if(readEeprom(16*eNum+3) < 24)
                    {
        ...
        else if(isCommand(&data, "setting", 0))
        {
            valid = true;
